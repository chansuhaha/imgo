...
namespace IMCC_ALL_OF_POE_GET
{
    public partial class MAIN_F : Form
    {
        ...
        private void MAIN_F_Load(object sender, EventArgs e)
        {
            ...
            #region 공통 저장용 클래스 로드
            CommonData.CommonDataInit(this);
            #endregion

            #region 로그 리스트 콘트롤 설정
            this.LogList_Init();
            #endregion
        ...
        {
            this.lblAvailable.Text = "- 사용가능한 비동기 I/O 쓰레드 수 : ";
            this.lblMax.Text = "- 사용가능한 작업쓰레드 수: ";
            this.lblUsedThread.Text = "- 사용중인 쓰레드 수 :";
            this.lblWorkerThread.Text = "- 최대 작업자 쓰레드수 :";
            this.lblCompletitionThread.Text = "- 최대 비동기 I/O 쓰레드수 :";
        }

        #region 그리드 Style 설정 선언
        ...
        #endregion 감지기 Model 설정

        #region 통신 시작, 종료버튼 이벤트 처리
        ...
        #endregion 통신 시작, 종료버튼 이벤트 처리

        #region 통신 시작 시 콘트롤 처리 ( 숨길지 말지)
        ...
        #endregion 통신 시작 시 콘트롤 처리 ( 숨길지 말지)
            ...
            try
            {
                while (CommonData.boProgramRun)
                {
                    try
                    {
                        ...
                        SetTextBox(this.lblWorkerThread, "- 최대 작업자 쓰레드수 :  " + iWorkerThread.ToString());
                        SetTextBox(this.lblCompletitionThread, "- 최대 비동기 I/O 쓰레드수 :  " + iCompletionThread.ToString());

                        ThreadPool.GetAvailableThreads(out iWorkth, out iComP);

                        SetTextBox(this.lblMax, "- 사용가능한 작업쓰레드 수: " + iWorkth.ToString());
                        SetTextBox(this.lblAvailable, "- 사용가능한 비동기 I/O 쓰레드 수 :  " + iComP.ToString());

                        SetTextBox(this.lblUsedThread, "- 사용중인 쓰레드 수 :  " + (iWorkerThread - iWorkth).ToString());
                    ...
                    {
                        SetTextBox(this.lblWorkerThread, "오류발생 :  " + ex.Message);
                    }
                }
            }
            catch { }
        ...
        private void btnPointAdd_Click(object sender, EventArgs e)
        {
            ...
            int pointCnt = GetPointPerDetector(); // 감지기당 Addres 점유율 수
            int iUseCnt = this.getChannelCount(sDetector); // 실제 사용 포인트 수
        ...
        private bool Map_Initilized()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < iCount ; i++)
                {
                    ...
                    // 기존에는 4채널은 전부 Y, 1채널은 첫번째꺼만 Y 로 셋팅 
                    node.VALID_YN = iUseCnt >= node.Channel ? "Y" : "N";

                    // 2019.07.29 == 요청사항 반영 최초 무조건 'N'
                    // node.VALID_YN = "N";
